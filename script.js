// Importar funciones de Firebase SDK v9+
...
let allPharmacies = []; 
let unsubscribe; // Para detener la escucha de datos al cerrar sesión
...
onAuthStateChanged(auth, user => {
    if (user) {
        ...
        if (unsubscribe) {
            unsubscribe(); // Detener la escucha de datos de Firestore
        }
    }
});
...
loginForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (error.code === 'auth/invalid-email' || error.code === 'auth/user-not-found') {
            loginError.textContent = 'El correo electrónico no está registrado.';
...
function initializeCRM() {
    const q = query(pharmaciesCollectionRef, orderBy('nombre'));
    unsubscribe = onSnapshot(q, (snapshot) => {
        loadingIndicator.style.display = 'block';
        tableBody.innerHTML = '';
        allPharmacies = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        ...
        // Mostrar/ocultar la sección de carga inicial
        if(allPharmacies.length > 0) {
            uploadSection.style.display = 'none';
        } else {
            uploadSection.style.display = 'flex'; // Usar flex para el layout interno
        }

    }, (error) => {
        console.error("Error al obtener farmacias: ", error);
        loadingIndicator.textContent = "Error al cargar los datos. Revisa las reglas de seguridad y la consola.";
    });
}
...
function renderTable(pharmacies) {
    ...
    if (pharmacies.length === 0) {
        ...
        td.colSpan = 5;
        td.textContent = 'No se encontraron farmacias con los filtros actuales.';
        ...
        pharmacies.forEach(pharmacy => {
            ...
            tr.innerHTML = `
                ...
                <td>${pharmacy.cadena}</td>
                <td>${pharmacy.direccion}</td>
                <td>
                    <button class="status-btn ${pharmacy.visitado.toLowerCase()}" data-id="${pharmacy.id}" data-field="visitado" data-current-status="${pharmacy.visitado}">${pharmacy.visitado}</button>
                </td>
                <td>
                    <button class="status-btn ${pharmacy.transferencia.toLowerCase()}" data-id="${pharmacy.id}" data-field="transferencia" data-current-status="${pharmacy.transferencia}">${pharmacy.transferencia}</button>
                </td>
            `;
            tableBody.appendChild(tr);
        });
    }
    recordCount.textContent = `${pharmacies.length} de ${allPharmacies.length} registros encontrados.`;
}
...
function populateCadenaFilter() {
    ...
    filterCadena.innerHTML = '<option value="">Cadena (Todas)</option>';
...
function applyFilters() {
    ...
    const filteredPharmacies = allPharmacies.filter(p => {
        const matchesSearch = p.nombre.toLowerCase().includes(searchTerm) || p.direccion.toLowerCase().includes(searchTerm);
        const matchesVisitado = !visitadoStatus || p.visitado === visitadoStatus;
        const matchesTransferencia = !transferenciaStatus || p.transferencia === transferenciaStatus;
        const matchesCadena = !cadenaFilter || p.cadena === cadenaFilter;
...
btnResetFilters.addEventListener('click', () => {
    searchBox.value = '';
    filterVisitado.value = '';
    filterTransferencia.value = '';
    filterCadena.value = '';
    applyFilters();
});
...
tableBody.addEventListener('click', async (e) => {
    if (e.target.classList.contains('status-btn')) {
        ...
        try {
            await updateDoc(docRef, { [field]: newStatus });
        } catch (error) {
...
btnUpload.addEventListener('click', async () => {
    if (!confirm('¿Estás seguro de que quieres cargar los datos iniciales? Esta acción solo debe realizarse una vez.')) return;
    
    const initialData = `Farmacia	Cadena	Direccion
...
26778 - DROGUERIAS GLOBALFARMA LA CUMBRE	COOPIDROGAS	CR 9 E # 28 - 90 BRR LA CUMбре - FLORIDABLANCA
...
VICTORIA DE LOS REYES	DROSAN LTDA	CL 67 18-25-BUCARAMANGA`;
    ...
    const pharmaciesToUpload = parseAndCleanData(initialData);
    ...
    pharmaciesToUpload.forEach(pharmacy => {
        const docRef = doc(pharmaciesCollectionRef); // Firestore genera un ID único
    ...
    try {
        await batch.commit();
        alert(`${pharmaciesToUpload.length} farmacias cargadas exitosamente a Firebase.`);
        uploadSection.style.display = 'none';
    } catch (error) {
        alert('Hubo un error al cargar los datos. Revisa la consola para más detalles.');
        console.error("Error al cargar datos en lote: ", error);
        btnUpload.disabled = false;
        btnUpload.textContent = 'Reintentar Carga';
    }
});

function parseAndCleanData(rawData) {
    ...
    lines.slice(1).forEach(line => { 
        ...
        if (parts.length >= 3) {
            ...
            const uniqueKey = `${nombre}-${cadena}-${direccion}`;
